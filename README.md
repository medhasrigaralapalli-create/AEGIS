...
- Full action range beyond scale up/down — resize (vertical, for memory/CPU-bound services), stop (for idle, explicitly stoppable workloads), and delay a batch workload (peak-pricing jobs) — chosen by a real LLM (Groq, GPT-OSS 120B) reasoning over the actual metrics, not a lookup table, each with a post-action watch window that rolls back if the outcome doesn't hold
...
It decides which services to investigate first, which of five actions fits the situation (scale up/down, resize, stop, delay a batch job, or no-op), whether its own data is trustworthy enough to act on, and whether to roll back an action it already took. The Decision Agent runs on a real LLM (Groq, GPT-OSS 120B) that reasons over each service's raw metrics rather than matching against fixed thresholds — confirmed in test runs by zero fallback triggers and correct handling of ambiguous cases (e.g. distinguishing `resize` from `scale_up`, and `delay_batch_workload` from `stop`, using CPU/memory rather than request count for batch jobs). If the safety gate rejects a proposal for being too large, the agent reasons about why and retries with the largest safe step on its own — a self-correction loop that holds regardless of which model is generating the proposal.

**Why wouldn't a fixed script, if-else rules, or a simple chatbot be enough?**
A fixed script can't reason about competing signals (low CPU but rising traffic, stale vs fresh data, one failed action needing a different fallback) or explain its reasoning in plain language. The decision space is conditional and combinatorial — rules would multiply endlessly and still miss cases like partial data or unexpected constraint conflicts. We proved this concretely: a batch job's request-per-minute count is a misleading idle signal (it reads 0 even mid-job), so a threshold rule would wrongly stop active work — the LLM correctly used CPU/memory instead once given that reasoning context.
...
- **Investigator Agent:** Reads service metrics (CPU, memory, traffic, latency, cost, recent deployment events) and checks how fresh each observation is; flags services worth acting on and holds off right after a recent deployment. Deterministic Python (no LLM call) — freshness rules don't need model reasoning, just a clear TTL policy per metric. Talks to the Service Metrics API and the Decision Agent.
- **Decision Agent:** Given a flagged service, proposes one of five actions (scale up, scale down, resize, stop, delay a batch workload, or no-op) with a predicted latency/cost outcome and a rollback condition. Calls a real LLM — Groq running `openai/gpt-oss-120b` — with a system prompt that explains the non-obvious action distinctions (e.g. why request count is a bad idle signal for batch jobs). Falls back to transparent rule-based heuristics if the API key is missing or the call fails for any reason, so the pipeline never crashes mid-demo. Talks to the Safety Gate.
- **Verifier Agent:** After execution, re-reads metrics during the watch window and decides whether the outcome matches the prediction or needs a rollback. Deterministic Python (no LLM call) — this is a threshold comparison against the SLO, not a reasoning task. Talks to the Executor and the Regret Ledger.
...
- **Web Dashboard (Streamlit, custom design system):** Shows current service state, proposed actions, and the action log, with a light/dark theme toggle and a visible "how it works" pipeline diagram at the top. Used by the user/judges.
...
| Layer | Technology |
|---|---|
| Frontend / Interface | Streamlit, with a custom CSS design system (Inter + JetBrains Mono, light/dark theme toggle) |
| Backend | Python — plain functions/modules (`pipeline.py` as orchestrator), not a framework |
| Agent Framework | Custom Python orchestration, not a framework like LangGraph/CrewAI — chosen so the Safety Gate could be a hard, ordinary function call in the loop rather than a graph node a framework might treat as optional or skippable |
| LLM Provider (Decision Agent only) | Groq, running `openai/gpt-oss-120b` — free tier, fast inference, no billing setup needed for a hackathon build. Swapping providers is a one-file change (`decision_agent.py`) since every other module only depends on the output shape, not which model produced it |
| Key management | `python-dotenv` — loads `GROQ_API_KEY` from a local `.env` file (excluded via `.gitignore`); an `.env.example` template is committed instead so the real key is never exposed |
| Database / Storage | SQLite — simple, zero-setup, enough for the Regret Ledger and action history in a hackathon build |
| Hosting | Local machine for development |
...
**Working:**
- Real LLM reasoning in the Decision Agent (Groq, GPT-OSS 120B) — verified across a full test run with zero fallback triggers, meaning every decision came from genuine model reasoning, not the heuristic backup
- Correct handling of ambiguous action choices via prompt engineering: `resize` vs `scale_up/down` (bottleneck is instance shape, not count) and `delay_batch_workload` vs `stop` (CPU/memory signal actual work, not request count, for batch jobs)
...
- Safety Gate enforcing min/max instances, blast-radius limit, SLO breach prevention, and protected-service rules — confirmed to work identically regardless of whether the Decision Agent is running on the LLM or the heuristic fallback
- Self-correction: an oversized proposal rejected by the Safety Gate is automatically retried at the largest safe step — confirmed working with the real LLM (e.g. the model itself proposed 1 instance for `reports-worker`, got rejected, and the pipeline retried at 2)
- Failed-action handling: a forced `capacity_unavailable` error triggers an automatic fallback to a smaller achievable step
- Post-action watch window that compares predicted vs. observed latency and rolls back on an SLO breach — in one test run, predicted and observed latency landed within 0.1% of each other
- Restyled dashboard (Streamlit, custom CSS design system) with a light/dark theme toggle and a visible pipeline diagram
...
- The `resize` vs `scale` and `delay` vs `stop` distinctions are reasoned about via prompt guidance, not hard-coded — usually but not 100% guaranteed correct on edge cases; an early run did hit a response-truncation issue (fixed by raising the LLM's output token limit)
- If the LLM call fails for any reason (no key, network issue, rate limit), the pipeline falls back to rule-based heuristics automatically rather than crashing — this fallback path is intentional and was demonstrated, not a leftover placeholder
...
Two things together: the self-correction loop (a proposal gets rejected by the deterministic Safety Gate, the agent reasons about *why*, and retries with a smaller safe step), and proof that the reasoning is genuine — the same scenario run twice produces slightly different wording (temperature-based variance), which a hardcoded rule never would.
