...
- Deterministic safety envelope (min/max instances, blast-radius limit, protected services) that gates every AI proposal
- Post-action watch window with automatic rollback if latency/error rate/availability degrades

**What are you deliberately NOT doing? (Optional)**
Not integrating a real cloud provider (AWS/GCP/Azure) — using a simulated service/environment API as the problem statement allows. Not building multi-cloud or multi-region support.
...
It decides which services to investigate first, which metrics matter for a given request, which action (scale up/down, resize, stop, defer, no-op) fits the situation, whether its own data is trustworthy enough to act on, and whether to roll back an action it already took — without a human confirming each step.
...
- **Investigator Agent:** Reads service metrics (CPU, memory, traffic, latency, cost) and checks how fresh each observation is; flags services worth acting on. Uses Claude Sonnet for its reasoning over mixed structured/unstructured signals. Talks to the Service Metrics API and the Decision Agent.
- **Decision Agent:** Given a flagged service, proposes one action (scale up/down, resize, stop, defer, no-op) with a predicted latency/cost outcome and a rollback condition. Uses Claude Sonnet for cost/latency trade-off reasoning. Talks to the Safety Gate.
- **Verifier Agent:** After execution, re-reads metrics during the watch window and decides whether the outcome matches the prediction or needs a rollback. Uses Claude Haiku, since this is a narrower comparison task. Talks to the Executor and the Regret Ledger.
...
Every proposed action must pass a deterministic Safety Gate before execution — the LLM never calls the cloud API directly. If a post-action watch window shows the outcome missed its prediction (e.g. latency rose instead of held), the Verifier Agent triggers an automatic rollback without asking the LLM again.
...
**Working:**
- Waste detection across multiple mocked services
- Freshness check that blocks stale-data decisions
- Safety Gate enforcing min/max instances and blast radius

**Partly working, mocked, or hard-coded:**
- Cloud API calls are mocked against sample JSON, not a real provider
- Latency prediction uses a simplified formula, not a trained model

**Not working or not built yet:**
- Multi-service concurrent action coordination
- Real-time streaming metrics (current build uses periodic polling)

**What we'd most like to be judged on:**
The safety loop — freshness rejection, pre-action prediction, and automatic rollback after a deliberately bad decision — since that's the part that separates this from a simple "if CPU is low, scale down" script.
